...
// the Panel. When a server process is started, Agent communicates with the
// Panel to fetch the latest build information as well as get all the details
// needed to parse the given Egg.
//
// This means we do not need to hit Agent each time part of the server is
// updated, and the Panel serves as the source of truth at all times. This also
// means if a configuration is accidentally wiped on Agent we can self-recover
// without too much hassle, so long as Agent is aware of what servers should
// exist on it.
...
// when determining if the credentials provided to Agent are valid.
...
// instance. This sets what Agent is looking for to mark a server as done

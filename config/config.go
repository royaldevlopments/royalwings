...
import (
	...
	"github.com/royalwings/system"
)

const DefaultLocation = "/etc/royalwings/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the royalwings data is stored at.
	RootDirectory string `default:"/var/lib/royalwings" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other agent events are logged.
	LogDirectory string `default:"/var/log/royalwings" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/royalwings/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/royalwings/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/royalwings/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for RoyalWings installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/royalwings" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"royalwings" yaml:"username"`
	...
	Passwd struct {
		...
		Directory string `yaml:"directory" default:"/run/royalwings/etc"`
	} `yaml:"passwd"`
	...
	MachineID struct {
		...
		// specified here. If you are using SELinux, you will need to make sure this file has the
		// correct SELinux context in order for containers to use it.
		Directory string `yaml:"directory" default:"/run/royalwings/machine-id"`
	} `yaml:"machine_id"`
...
type Configuration struct {
	...
	AppName string `default:"RoyalWings" json:"app_name" yaml:"app_name"`
...
// EnsureRoyalWingsUser ensures that the RoyalWings core user exists on the
...
func EnsureRoyalWingsUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "royalwings")
	...
	log.WithField("username", _config.System.Username).Info("checking for royalwings system user")
...
func EnableLogRotation() error {
	...
	if _, err := os.Stat("/etc/logrotate.d/royalwings"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	f, err := os.Create("/etc/logrotate.d/royalwings")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/royalwings.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP royalwings.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err

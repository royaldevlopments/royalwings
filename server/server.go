...
import (
	...
	"github.com/royalwings/events"
	"github.com/royalwings/remote"
	"github.com/royalwings/server/filesystem"
...
type Server struct {
	...
	// fetched from the RoyalWings Server instance each time the server process is
...
// Sync syncs the state of the server on the Panel with Agent. This ensures that
// we're always using the state of the server from the Panel and allows us to
// not require successful API calls to Agent to do things.
//
// This also means mass actions can be performed against servers on the Panel
// and they will automatically sync with Agent when the server is started.
...
// instance on Agent. This includes the information needed by the Panel in order

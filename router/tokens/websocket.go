...
// The time at which Agent was booted. No JWT's created before this time are allowed to
...
// before Agent was booted, or because we have denied all tokens with the same JTI
...
func (p *WebsocketPayload) Denylisted() bool {
	...
	// If the time that the token was issued is before the time at which Agent was booted
	// then the token is invalid for our purposes, even if the token "has permission".
	if p.IssuedAt.Time.Before(agentBootTime) {
		return true
	}
